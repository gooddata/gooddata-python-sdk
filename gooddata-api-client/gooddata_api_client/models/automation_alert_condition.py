...
AUTOMATIONALERTCONDITION_ONE_OF_SCHEMAS = ["ComparisonWrapper", "RangeWrapper", "RelativeWrapper"]
...
class AutomationAlertCondition(BaseModel):
    ...
    actual_instance: Optional[Union[ComparisonWrapper, RangeWrapper, RelativeWrapper]] = None
    one_of_schemas: Set[str] = { "ComparisonWrapper", "RangeWrapper", "RelativeWrapper" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in AutomationAlertCondition with oneOf schemas: ComparisonWrapper, RangeWrapper, RelativeWrapper. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in AutomationAlertCondition with oneOf schemas: ComparisonWrapper, RangeWrapper, RelativeWrapper. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into AutomationAlertCondition with oneOf schemas: ComparisonWrapper, RangeWrapper, RelativeWrapper. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into AutomationAlertCondition with oneOf schemas: ComparisonWrapper, RangeWrapper, RelativeWrapper. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], ComparisonWrapper, RangeWrapper, RelativeWrapper]]:
