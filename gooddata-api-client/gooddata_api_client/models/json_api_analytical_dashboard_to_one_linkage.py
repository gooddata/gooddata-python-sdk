...
class JsonApiAnalyticalDashboardToOneLinkage(BaseModel):
    ...
    def actual_instance_must_validate_oneof(cls, v):
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in JsonApiAnalyticalDashboardToOneLinkage with oneOf schemas: JsonApiAnalyticalDashboardLinkage. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
    ...
    def from_json(cls, json_str: Optional[str]) -> Self:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into JsonApiAnalyticalDashboardToOneLinkage with oneOf schemas: JsonApiAnalyticalDashboardLinkage. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
