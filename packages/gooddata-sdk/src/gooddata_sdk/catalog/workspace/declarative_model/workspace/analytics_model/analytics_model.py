...
class CatalogDeclarativeAnalytics(Base):
    ...
    def validate(
        ...
        *,
        ...
        """Validate this model as far as the files allow, collecting everything into one report.

        Two levels run here. Each visualization is checked on its own -- required keys, and
        the references its sorts and filters make into its own buckets. Then the model is
        checked as a set: ids repeated within a collection, dashboards naming objects the
        layout does not carry, and metric definitions that form a cycle.

        What is *not* checked is whether the metrics, labels and datasets these objects name
        exist anywhere: that needs a workspace, and lives in
        :class:`gooddata_sdk.catalog.validation.service.ValidationService`.
        ...
        report.extend(validate_layout(self))
        return report
