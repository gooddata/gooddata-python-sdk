...
def resolve_references(
    ...
    *,
    ...
    scope: str = "the target workspace or anything it inherits from",
    ...
    Args:
        ...
        scope: names, in the message, what the reference was resolved against. The default
            describes a workspace; a caller resolving against a layout's own logical model
            must say so, or the finding claims a workspace was consulted when none was.
        ...
        if reference.obj_id not in effective_ids:
            ...
            findings.append(
                Finding(
                    ...
                    message=f"{reference.obj_id} does not exist in {scope}",
                    object_id=object_id,
