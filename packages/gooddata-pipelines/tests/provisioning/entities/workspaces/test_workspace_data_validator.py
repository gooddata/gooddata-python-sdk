...
def test_check_basic_integrity_on_valid_data(validator) -> None:
    ...
    parent_workspaces, parent_wdf_map = validator._check_basic_integrity(source_group)
...
def test_check_basic_integrity_missing_workspace_id(validator) -> None:
    ...
    try:
        ...
        assert isinstance(e, WorkspaceException)
        assert e.error_message == "Workspace ID is not defined for parent parent_id_1"
...
def test_check_basic_integrity_missing_wdf_id(validator) -> None:
    ...
    try:
        ...
        assert isinstance(e, WorkspaceException)
        assert e.error_message == "WDF values are provided but WDF ID is not defined."
...
def test_check_basic_integrity_missing_wdf_values(validator) -> None:
    ...
    try:
        ...
        assert isinstance(e, WorkspaceException)
        assert e.error_message == "WDF ID is defined but no WDF values are provided"
        assert e.workspace_id == "workspace_id_1"
...
def test_check_basic_integrity_duplicates_warning(validator, caplog) -> None:
    ...
    warning_log_found = any(
        "Duplicate combinations of parent_id, workspace_id, wdf_id exist in the source data." in message
        for message in caplog.messages
    )

    assert warning_log_found, "Warning log for duplicate combinations not found in logs."
