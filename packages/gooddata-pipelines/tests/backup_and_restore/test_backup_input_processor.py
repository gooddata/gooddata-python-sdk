...
def backup_input_processor(mock_gooddata_api, mock_logger):
    ...
    processor.hierarchy_endpoint = "/fake/hierarchy?filter=parent.id=={parent_id}"
...
def test_paginate_calls_fetch_page_and_process_data(backup_input_processor, monkeypatch):
    ...
    resp1 = WorkspaceResponse(data=[ws1], meta=Meta(hierarchy=None, page=None), links=links1)
    resp2 = WorkspaceResponse(data=[ws2], meta=Meta(hierarchy=None, page=None), links=links2)
...
def test_get_workspaces_to_backup_empty_org(backup_input_processor, monkeypatch, capsys):
