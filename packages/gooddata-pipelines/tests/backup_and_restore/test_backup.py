...
def test_archive_gooddata_layouts_to_zip(backup_manager):
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        backup_manager._archive_gooddata_layouts_to_zip(str(Path(tmpdir, "services")))
...
def test_store_user_data_filters(backup_manager):
    ...
    user_data_filter2 = os.path.isfile(Path(f"{user_data_filter_folderlocation}/datafilter2.yaml"))
    user_data_filter4 = os.path.isfile(Path(f"{user_data_filter_folderlocation}/datafilter4.yaml"))
...
def test_file_upload(backup_manager, s3, s3_bucket):
    backup_manager.storage.export(f"{TEST_DATA_SUBDIR}/test_exports", "services")
    s3.Object(
        S3_BUCKET,
...
@mock.patch("gooddata_pipelines.backup_and_restore.backup_manager.BackupManager._get_workspace_export")
@mock.patch("gooddata_pipelines.backup_and_restore.backup_manager.BackupManager._archive_gooddata_layouts_to_zip")
...
def test_process_batch_retries_on_exception(
    ...
    assert captured.out.startswith("Exception encountered while processing a batch. Retrying")
    backup_manager.storage.export.assert_called_once()
