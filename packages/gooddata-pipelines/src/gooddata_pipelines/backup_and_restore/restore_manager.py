...
class RestoreManager(BaseManager):
    ...
    def _extract_zip_archive(self, file_to_extract: Path, destination: Path) -> None:
    ...
    def _check_workspace_is_valid(self, workspace_root_dir_path: Path) -> None:
        ...
        if not workspace_root_dir_path.exists() or not workspace_root_dir_path.is_dir():
            self.logger.error(
        ...
        if am_path not in children or ldm_path not in children or udf_path not in children:
            self.logger.error(f"{DirNames.AM} or {DirNames.LDM} directory missing in the " + "workspace hierarchy. ")
            raise RuntimeError(f"{DirNames.AM} or {DirNames.LDM} directory missing.")

    @log_and_reraise_exception("Failed to load workspace declaration.")
    def _load_workspace_layout(self, workspace_root_dir_path: Path) -> WorkspaceModel:
    ...
    def _load_user_data_filters(self, workspace_root_dir_path: Path) -> dict:
        ...
        user_data_filters_folder = os.path.join(workspace_root_dir_path, DirNames.UDF)
    ...
    def _put_workspace_layout(self, workspace_id: str, workspace_model: WorkspaceModel) -> None:
        """Puts the workspace layout into GoodData."""
        self._api._sdk.catalog_workspace_content.put_declarative_ldm(workspace_id, workspace_model.logical_data_model)
    ...
    def _put_user_data_filters(self, workspace_id: str, user_data_filters: dict) -> None:
        """Puts the user data filters into GoodData workspace."""
        response = self._api.put_user_data_filters(workspace_id, user_data_filters)
        self._api.raise_if_response_not_ok(response)

    def _load_and_put_filter_views(self, workspace_id: str, workspace_root_dir_path: Path) -> None:
        ...
        for file in Path(workspace_root_dir_path / "filter_views").iterdir():
            filter_view_content: dict[str, Any] = dict(self.yaml_utils.safe_load(file))
            filter_view: CatalogDeclarativeFilterView = CatalogDeclarativeFilterView.from_dict(filter_view_content)
            filter_views.append(filter_view)

        if filter_views:
            self._api._sdk.catalog_workspace.put_declarative_filter_views(workspace_id, filter_views)

    def _load_and_post_automations(self, workspace_id: str, workspace_root_dir_path: Path) -> None:
        """Loads automations from specified json file and creates them in the workspace."""
        # Load automations from JSON
        path_to_json: Path = Path(workspace_root_dir_path, "automations", "automations.json")
    ...
    def _delete_all_automations(self, workspace_id: str) -> None:
        ...
        automations: list[CatalogDeclarativeAutomation] = self._api._sdk.catalog_workspace.get_declarative_automations(
            workspace_id
        )
        for automation in automations:
    ...
    def _post_automation(self, workspace_id: str, automation: dict) -> None:
        ...
        if attributes.get("schedule") and attributes["schedule"].get("cronDescription"):
        ...
        if not response.ok:
            self.logger.error(f"Failed to post automation ({response.status_code}): {response.text}")

    def _restore_backup(self, workspace_to_restore: WorkspaceToRestore, tempdir_path: Path) -> None:
        ...
        try:
            self.storage.get_ws_declaration(workspace_to_restore.path, str(zip_target))
            self._extract_zip_archive(zip_target, tempdir_path)
            self._check_workspace_is_valid(src_path)
            workspace_model: WorkspaceModel = self._load_workspace_layout(src_path)
            ...
            self._put_user_data_filters(workspace_to_restore.id, user_data_filters)
            ...
            self.logger.info(f"Finished backup restore of {workspace_to_restore.id} from {workspace_to_restore.path}.")
