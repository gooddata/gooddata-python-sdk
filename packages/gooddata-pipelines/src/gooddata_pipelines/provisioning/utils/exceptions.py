...
class ContextException(Exception, AttributesMixin):
    def __init__(self, message: str, *context_objects: object, **kwargs: str) -> None:
...
class WorkspaceDataIntegrityException(WorkspaceException):
    def __init__(self, message: str, *context_objects: object, **kwargs: str) -> None:
        ...
        self.workspace_id: str = self.attributes.get("workspace_id", "NA")
        self.workspace_name: str | None = self.attributes.get("workspace_name", None)
        self.wdf_id: str | None = self.attributes.get("wdf_id", None)
        self.wdf_values: str | None = self.attributes.get("wdf_values", None)
        self.api_endpoint: str = self.attributes.get("api_endpoint", "workspace_data_validation")
...
class BaseUserException(ProvisioningException):
    def __init__(self, message: str, *context_objects: object, **kwargs: str) -> None:
        ...
        self.workspace_id: str | None = self.attributes.get("workspace_id", None)
        self.user_id: str | None = self.attributes.get("user_id", None)
        self.user_group_id: str | None = self.attributes.get("user_group_id", None)
        self.api_endpoint: str = self.attributes.get("api_endpoint", "user_provisioning")
