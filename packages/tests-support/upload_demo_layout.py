...
# Staging overrides for data source connection
# Local Docker: postgres:5432, user=postgres
# Staging k8s:  cnpg-cluster-pooler:5432, user=tiger
STAGING_DS_URL = "jdbc:postgresql://cnpg-cluster-pooler:5432/tiger?sslmode=prefer"
STAGING_DS_USERNAME = "tiger"
STAGING_DS_PASSWORD = "passw0rd"
...
def patch_data_sources_for_staging(data_sources):
    """Patch data source fixtures with staging PostgreSQL connection details."""
    for ds in data_sources.get("dataSources", []):
        ds["url"] = STAGING_DS_URL
        ds["username"] = STAGING_DS_USERNAME
        ds["password"] = STAGING_DS_PASSWORD
...
def update_layout():
    ...
    # Patch data source connection for staging
    if staging:
        print("Patching data sources for staging...", flush=True)
        data_sources = patch_data_sources_for_staging(data_sources)
