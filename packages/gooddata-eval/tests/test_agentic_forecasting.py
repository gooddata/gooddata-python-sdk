...
#
# Every other multi-run kind takes `gate` and asks `gate_passed(...)`. Forecasting computed
# pass^K and then ignored it, so `--gate power` on a flaky item reported a pass on the
# strength of one good run out of K -- the precise case the gate exists to catch.


def _two_runs(gate=None):
    """One passing run then one failing one, which pass@K clears and pass^K must not."""
    passing = _chat([_tc("create_adhoc_visualization", _viz_args()), _tc("execute_forecast", {}, _OK_FORECAST)])
    failing = _chat([])
    client = MagicMock()
    client.create_conversation.side_effect = ["conv-1", "conv-2"]
    client.send_message.side_effect = [passing, failing, failing, failing, failing, failing]
...
def test_power_gate_fails_an_item_where_only_some_runs_passed():
    with pytest.raises(ForecastingAssertionError) as excinfo:
