...
def evaluate_agentic_dashboard_skill(
    ...
    def _run_detail(run: DashboardRunResult) -> dict:
        ...
        for this kind that means the per-check breakdown and the reported failures, which is
        what separates a run that never activated the skill from one that drafted the wrong
        dashboard.
        """
        return {
