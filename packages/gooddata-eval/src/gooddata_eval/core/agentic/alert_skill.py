    ...
    def _run_once(conv_id: str) -> AlertRunResult:
        ...
        try:
            ...
            for _iteration in range(max_iterations):
                # Counts the turn as attempted, before the call: a turn the chat faulted on
                # still consumed one of max_iterations. `turns` below counts only the turns
                # that came back, so the two differ by design on a CHAT_ERROR exit.
...
def _run_detail(run: AlertRunResult, max_iterations: int) -> dict:
    ...
    for this kind that means the full per-field check breakdown, the arguments the run sent
    and why its loop stopped.
    """
    ev = run.eval
    return {
