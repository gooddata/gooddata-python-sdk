...
from gooddata_sdk.exec_model import Attribute, SimpleMeasure, ObjId, Measure, Filter, exec_model_to_api_model
...
ValidObjectTypes = Union[Attribute, Measure, Filter, CatalogLabel, CatalogFact, CatalogMetric]
ValidObjectsInputType = Union[ValidObjectTypes, list[ValidObjectTypes], ExecutionDefinition]


class Catalog:
    def __init__(self, valid_obj_fun, datasets: list[CatalogDataset],
    ...
    def catalog_with_valid_objects(self, ctx: ValidObjectsInputType):
        ...
        new_datasets = [non_empty for non_empty in [d.filter_dataset(valid_objects) for d in self.datasets] if
...
def _create_catalog(valid_obj_fun, datasets, attributes, metrics) -> Catalog:
    ...
    catalog_facts = dict([(fact['id'], CatalogFact(fact)) for fact in dataset_sideloads.all_for_type('fact')])
    ...
    for attr in attributes.data:
        ...
        catalog_attributes[attr_id] = CatalogAttribute(attr,
                                                       [CatalogLabel(attribute_sideloads.find(label_id)) for label_id in
                                                        label_ids])
    ...
    for dataset in datasets.data:
        ...
        catalog_datasets[dataset_id] = CatalogDataset(dataset,
                                                      [catalog_attributes[attr_id_obj['id']] for attr_id_obj in
                                                       attribute_ids],
                                                      [catalog_facts[fact_id_obj['id']] for fact_id_obj in fact_ids]
                                                      )

    return Catalog(valid_obj_fun=valid_obj_fun,
                   datasets=list(catalog_datasets.values()), metrics=catalog_metrics)
...
def _prepare_afm_for_availability(items: list[ValidObjectTypes]):
    ...
    return exec_model_to_api_model(attributes=attributes, measures=measures, filters=filters)
...
class CatalogService:
    ...
    def get_full_catalog(self, workspace_id: str) -> Catalog:
        ...
        :return:
        """
        get_datasets = functools.partial(self._api.get_all_entities_datasets, workspace_id,
                                         include=['attributes', 'facts'], _check_return_type=False)

        get_attributes = functools.partial(self._api.get_all_entities_attributes, workspace_id, include=['labels'],
    ...
    def compute_valid_objects(self, workspace_id: str, ctx: ValidObjectsInputType) -> ValidObjects:
        ...
        if isinstance(ctx, ExecutionDefinition):
            afm = exec_model_to_api_model(attributes=ctx.attributes, measures=ctx.measures, filters=ctx.filters)
        else:
