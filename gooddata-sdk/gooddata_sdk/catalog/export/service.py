...
class ExportService:
    ...
    def _get_exported_content(
        ...
        assert timeout > 0 and retry > 0, f"Timeout value '{timeout}' or retry value '{retry}' is negative."
    ...
    def _export_common(
        ...
        content = self._get_exported_content(workspace_id, export_id, get_func)
        with open(file_path, "wb") as f:
            f.write(content)

    def export_pdf(
        self, workspace_id: str, dashboard_id: str, file_name: str, store_path: Union[str, Path] = Path.cwd()
    ) -> None:
        ...
        self._export_common(workspace_id, request, file_path, create_func, get_func)

    def export_tabular(
        self, workspace_id: str, export_request: ExportRequest, store_path: Union[str, Path] = Path.cwd()
    ) -> None:
        store_path = store_path if isinstance(store_path, Path) else Path(store_path)
        ...
        self._export_common(workspace_id, export_request.to_api(), file_path, create_func, get_func)
