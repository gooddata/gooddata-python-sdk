...
from gooddata_metadata_client.exceptions import (
    ApiKeyError,
    ApiAttributeError,
    ApiTypeError,
    ApiValueError,
...
def convert_js_args_to_python_args(fn):
    ...
    @wraps(fn)
    def wrapped_init(_self_loathing_, *args, **kwargs):
        """
        An attribute named `self` received from the api will conflicts with the reserved `self`
        parameter of a class method. During generation, `self` attributes are mapped
        to `_self_loathing_` in models. Here, we name `_self_loathing_` instead of `self` to avoid conflicts.
        """
        spec_property_naming = kwargs.get('_spec_property_naming', False)
        if spec_property_naming:
            kwargs = change_keys_js_to_python(kwargs, _self_loathing_ if isinstance(_self_loathing_, type) else _self_loathing_.__class__)
        return fn(_self_loathing_, *args, **kwargs)
    return wrapped_init
