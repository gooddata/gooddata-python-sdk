...
def test_create_groups_base_case() -> None:
    ...
    ids_in_both_systems, ids_to_delete, ids_to_create = (
        provisioner._create_groups(test_source_ids, test_panther_ids)
    )

    assert ids_in_both_systems == {"source_id_2", "source_id_3"}
    assert ids_to_delete == {"source_id_4"}
    assert ids_to_create == {"source_id_1"}
...
def test_create_groups_empty_sets() -> None:
    ...
    assert ids_in_both_systems == set()
    assert ids_to_delete == set()
    assert ids_to_create == set()
...
def test_create_groups_no_overlap() -> None:
    ...
    assert ids_in_both_systems == set()
    assert ids_to_delete == {"source_id_3", "source_id_4"}
    assert ids_to_create == {"source_id_1", "source_id_2"}
...
def test_create_groups_full_overlap() -> None:
    ...
    assert ids_in_both_systems == {"source_id_1", "source_id_2"}
    assert ids_to_delete == set()
    assert ids_to_create == set()
